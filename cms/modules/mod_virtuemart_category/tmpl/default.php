...
<ul class="VMmenu<?php echo $class_sfx ?>" id="<?php echo "VMmenu".$ID ?>">
	<?php foreach ($categories as $category) {
		...
		$Itemid = mod_virtuemart_category::getCatItemidUrlStr($category);
		$caturl = JRoute::_('index.php?option=com_virtuemart&view=category&virtuemart_category_id='.$category->virtuemart_category_id.$Itemid);
	...
	<li <?php echo $active_menu ?>>
		...
		<ul class="menu<?php echo $class_sfx; ?>">
			...
			foreach ($category->childs as $child) {
				...
				$Itemid = mod_virtuemart_category::getCatItemidUrlStr($child);
				$caturl = JRoute::_('index.php?option=com_virtuemart&view=category&virtuemart_category_id='.$child->virtuemart_category_id.$Itemid);
				...
				if(!empty($child->childs)){ ?>
					<ul class="menu<?php echo $class_sfx; ?>">
						...
						foreach ($child->childs as $child1) {
							$Itemid = mod_virtuemart_category::getCatItemidUrlStr($child1);
							$active_child_menu = 'class="VmClose"';
							$caturl = JRoute::_('index.php?option=com_virtuemart&view=category&virtuemart_category_id='.$child1->virtuemart_category_id.$Itemid);
