...
class mod_virtuemart_category {
	...
	static public function getCatItemidUrlStr($category){

		$Itemid = '';
		if(class_exists('vmrouterHelper') and !empty(vmrouterHelper::$menu)){

			if(!empty( vmrouterHelper::$menu['virtuemart_category_id'][$category->virtuemart_category_id][0])){
				$Itemid = vmrouterHelper::$menu['virtuemart_category_id'][$category->virtuemart_category_id][0];
			} else if (!empty(vmrouterHelper::$menu['virtuemart'])){
				$Itemid = vmrouterHelper::$menu['virtuemart'];
			} else {
				vmdebug('No menu found for category '.$category->virtuemart_category_id,vmrouterHelper::$menu);
			}
